...
function addNewWordSubmission(word) {
    ...
    //console.log($.inArray(("word :" + word), model.wordSubmissions.word));
...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // TODO 15
            for (var e in model.wordSubmissions) {
                if (model.wordSubmissions[e].word == word){
                  console.log(model.wordSubmissions[e]);
                  (model.wordSubmissions[e])['isRealWord'] = theAnswer;
                }
            }
...
function render() {
    ...
    //var wordChips = model.wordSubmissions.map(wordSubmissionChip(wordChip))
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        var scoreChip = $("<span></span>").text(wordScore(wordSubmission.word)).addClass("tag-primary").attr('id', 'scoreChip');

        // TODO 17
        // give the scoreChip appropriate text content
        if (wordSubmission.isRealWord == false ){
        scoreChip = $("<span></span>").text("X").addClass("tag-danger").attr('id', 'scoreChip');
        }
        // TODO 18
...
function wordScore(word) {
    ...
    var letterScores = letters.map(function(letter){
      //console.log(scrabblePointsForEachLetter[letter]);
